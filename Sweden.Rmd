...
```{r}
df %>% filter(date >="2020-12-01" ) %>%
  ggplot() + geom_col(aes(x=date,y=new_cases)) +
  labs(title="Cases: Plot of Tuesday - Friday Data.")

df %>% filter(date >="2020-12-01" ) %>%
  ggplot() + geom_area(aes(x=date,y=CMA)) +
...
df %>% filter(date >="2020-12-01") %>%
  ggplot() + geom_area(aes(x=date,y=DMA)) +
