...
df %>% filter(date >="2020-12-01" ) %>%
  ggplot() + geom_line(aes(x=date,y=CMA)) +
...
df %>% filter(date >="2020-12-01") %>%
  ggplot() + geom_line(aes(x=date,y=DMA)) +
...
```{r}
df %>% ggplot() + geom_col(aes(x=date,y=total_cases,col=total_cases)) +
  scale_y_continuous(labels = comma) +
  labs(title="Sweden Cumulative Cases by Date")

df %>% ggplot() + geom_col(aes(x=date,y=total_deaths,col=total_deaths)) +
  scale_y_continuous(labels = comma) +
