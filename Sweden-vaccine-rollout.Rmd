---
title: "Sweden's COVID-19 Vaccine Rollout"
...
### TWitch Live Stream Sunday - Friday at 8:00 PM/EDT(New York time):
* https://www.twitch.tv/davidjayjackson

## Sweden's COVID-19 Rollout
...
```
```{r}
VAC <- SWE  %>% filter(people_vaccinated >0) %>%
...
p1 <- VAC %>% ggplot() + geom_line(aes(x=date,y=total_vaccinations)) +
...
```{r}
p2 <- VAC %>% ggplot() + geom_line(aes(x=date,y=people_vaccinated,col="People Vacc.")) +
