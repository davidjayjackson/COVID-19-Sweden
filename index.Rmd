---
title: "SWeden's Battle With Covid-29"
...
mean(df$new_cases,na.rm=TRUE) *365
```

```{r}
mean(df$new_deaths,na.rm=TRUE)
mean(df$new_deaths,na.rm=TRUE) *365
```
...
db2$MAC <- movavg(db2$new_cases,14,type = "w")
db2$MAD <- movavg(db2$new_deaths,14,type = "w")
...
```{r}
df$PC <- (df$new_cases/10152828)*100000
...
```
```{r}
df$DPC <- (df$new_deaths/10152828) *100000
